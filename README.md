...
| 首页 | 当月总收入 / 总支出 / 结余 + 今日收支 + 月度预算进度条与超支提醒 + 最近 10 条账单预览 + 悬浮记账按钮 |
| 账单列表 | 时间倒序、按月分组（组头显示当月收支合计）、搜索（备注/分类名）+ 类型筛选 + 金额区间筛选 |
| 统计页 | 图表分析中心：分类占比饼图（Canvas 自绘，支出/收入/全部三态 + 日/周/月/年维度）、近 6 月收支柱状对比、近 12 月消费趋势折线 |
...
| 桌面小组件 | Glance 小组件：显示本月结余 / 收支 / 预算剩余，一键快捷记账，打开 App 自动刷新 |
