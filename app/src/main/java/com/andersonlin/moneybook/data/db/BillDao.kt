...
interface BillDao {
    ...
    /** 某账本全部账单，时间倒序 */
    @Query("SELECT * FROM bills WHERE ledgerId = :ledgerId ORDER BY dateEpochDay DESC, id DESC")
    fun getAllBills(ledgerId: Long): Flow<List<Bill>>

    /** 某账本最近 N 条账单（首页预览） */
    @Query(
        "SELECT * FROM bills WHERE ledgerId = :ledgerId " +
            "ORDER BY dateEpochDay DESC, id DESC LIMIT :limit"
    )
    fun getRecentBills(ledgerId: Long, limit: Int): Flow<List<Bill>>
    ...
    @Query(
        "SELECT type, SUM(amountCents) AS total FROM bills " +
            "WHERE ledgerId = :ledgerId AND dateEpochDay BETWEEN :startDay AND :endDay " +
            "GROUP BY type"
    )
    fun getMonthSummary(ledgerId: Long, startDay: Long, endDay: Long): Flow<List<MonthSum>>
    ...
    @Query(
        "SELECT categoryId, SUM(amountCents) AS total FROM bills " +
            "WHERE ledgerId = :ledgerId AND (:type = -1 OR type = :type) " +
    ...
    fun getMonthlyTotals(ledgerId: Long, startDay: Long, endDay: Long): Flow<List<MonthlyTotal>>
    ...
    fun getDailyTotals(ledgerId: Long, startDay: Long, endDay: Long): Flow<List<DailyTotal>>

    /** 某账本某天的收支合计 */
    @Query(
        "SELECT type, SUM(amountCents) AS total FROM bills " +
            "WHERE ledgerId = :ledgerId AND dateEpochDay = :day GROUP BY type"
    )
    fun getDaySummary(ledgerId: Long, day: Long): Flow<List<MonthSum>>

    /**
     * 某账本内搜索 / 筛选：类型过滤 + 关键字 + 金额区间，时间倒序。
    ...
    @Query(
        ...
        WHERE b.ledgerId = :ledgerId
          AND (:typeFilter = -1 OR b.type = :typeFilter)
    ...
    @Query("SELECT COUNT(*) FROM bills WHERE ledgerId = :ledgerId")
    suspend fun countByLedger(ledgerId: Long): Int
    ...
    /** 某账本的账单快照（CSV 导出用） */
    @Query(
        "SELECT * FROM bills WHERE ledgerId = :ledgerId ORDER BY dateEpochDay ASC, id ASC"
    )
    suspend fun getSnapshotForLedger(ledgerId: Long): List<Bill>
